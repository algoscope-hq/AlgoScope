...
import { generateJobSequencingSteps } from '../../algorithms/greedy/jobSequencingSteps'
import {
  getGreedySource,
  resolveGreedyLine,
...
const DEFAULT_JOBS = [
  { id: 'Job A', profit: 100, deadline: 2 },
  { id: 'Job B', profit: 19, deadline: 1 },
  { id: 'Job C', profit: 27, deadline: 2 },
  { id: 'Job D', profit: 25, deadline: 1 },
  { id: 'Job E', profit: 15, deadline: 3 },
]
...
export default function VisualizerPage() {
  ...
  const handleVisualize = () => {
    ...
    } else if (algorithm === 'jobsequencing') {
      const jobSteps = generateJobSequencingSteps(jobs)
      loadSteps(jobSteps, { autoPlay: !isStepMode })
    }
  }
  ...
  const handleAddJob = () => {
    const profit = Number(newJobProfit)
    const deadline = Number(newJobDeadline)
    if (isNaN(profit) || isNaN(deadline) || profit <= 0 || deadline <= 0) return

    const nextLetter = String.fromCharCode(65 + jobs.length)
    const newJob = {
      id: `Job ${nextLetter}`,
      profit,
      deadline: Math.min(9, deadline), // clamp max deadline to 9 to prevent UI overflow
    }
            ...
            <div className="flex min-w-0 min-h-0 flex-col gap-4">
              ...
              <div className="min-w-0">
                {algorithm === 'huffman' ? (
                  ...
                  <JobSequencingVisualizer currentStep={currentStep} />
                )}
              </div>
                      ...
                      <div className="bg-slate-950/60 p-3 rounded-2xl border border-slate-800/80 space-y-2">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <input
                            type="number"
                            value={newJobProfit}
                            onChange={(e) => setNewJobProfit(e.target.value)}
